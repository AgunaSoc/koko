...
goldflags="-X 'main.Buildstamp=$buildStamp' -X 'main.Githash=$gitHash' -X 'main.Goversion=$goVersion' -X 'github.com/atherlock/koko/pkg/koko.Version=$kokoVersion' -X 'github.com/atherlock/koko/pkg/config.CipherKey=$cipherKey'"
k8scmdflags="-X 'github.com/atherlock/koko/pkg/config.CipherKey=$cipherKey'"
# 下载依赖模块并构建
